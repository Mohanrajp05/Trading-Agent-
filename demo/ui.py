...
class Trader:
    ...
    def get_portfolio_value_chart(self):
        ...
        # A new account has no recorded report yet.  Plotly renders an empty
        # figure with misleading default axes, so show the actual starting
        # balance until the first report is recorded.
        if df.empty:
            df = pd.DataFrame(
                [{"datetime": pd.Timestamp.now(), "value": self.account.calculate_portfolio_value()}]
            )
