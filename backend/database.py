...
# Resolve the database from the project directory, rather than the process
# working directory.  The dashboard and the scheduler are often started from
# different directories; a relative path makes each process create/read a
# different database and produces an apparently empty dashboard.
DB = str(Path(__file__).resolve().parents[1] / "accounts.db")
