...
export class TraderPanel {
  ...
  constructor(state: TraderState, onSelect?: (name: string) => void) {
    this.state = state;
    ...
    this.root.setAttribute("aria-label", `Open details for ${name}`);
    this.root.addEventListener("click", () => onSelect?.(name));
